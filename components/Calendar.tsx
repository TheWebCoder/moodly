...
export default function Calendar({ demo, completeData, handleSetMood }: CalendarProps) {
  ...
  function handleIncrementMonth(val: number) {
    if (numericMonth + val < 0) {
      ...
      setSelectedMonth(monthsArr[monthsArr.length - 1] ?? 'December');
    } else if (numericMonth + val > 11) {
      setSelectedYear((curr) => curr + 1);
      setSelectedMonth(monthsArr[0] ?? 'January');
    } else {
      setSelectedMonth(monthsArr[numericMonth + val] ?? selectedMonth);
    }
  }
        ...
        {[...Array(numRows).keys()].map((row, rowIndex) => (
          <div key={rowIndex} className="grid grid-cols-7 gap-1">
            {dayList.map((dayOfWeek, dayOfWeekIndex) => {
              ...
              if (!dayDisplay || dayIndex < 1) {
                return <div className="bg-white" key={dayOfWeekIndex} />;
              }

              const color = demo
                ? gradients.indigo[baseRating[String(dayIndex)]]
                : dayIndex in data
              ...
              return (
                <div
                  ...
                  className={
                    'text-xs sm:text-sm border boder-solid p-2 flex items-center gap-2 justify-between rounded-lg ' +
