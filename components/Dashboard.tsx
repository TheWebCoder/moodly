...
export default function Dashboard() {
  ...
  async function handleSetMood(mood: number) {
    ...
    try {
      const newData: MoodData = userDataObj ? { ...userDataObj } : {};
      if (!newData[year]) {
        newData[year] = {};
      }
      ...
      console.log('Failed to set data: ' + (err instanceof Error ? err.message : err));
    }
  }
      ...
      <div className="grid sm:grid-cols-3 bg-indigo-50 text-indigo-500 p-4 gap-4 rounded-lg">
        {Object.keys(statuses).map((status, statusIndex) => (
          <div key={statusIndex} className="flex flex-col gap-1 sm:gap-2">
            ...
            <p
              className={
                'text-base sm: text-lg truncate ' + fugaz.className
              }
            >
              {statuses[status]}
              {status === 'num_days' ? ' 🔥' : ''}
